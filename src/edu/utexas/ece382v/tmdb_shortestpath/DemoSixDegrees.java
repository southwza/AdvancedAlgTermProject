...
public class DemoSixDegrees {
    public static void printPath(AgentNode target) {
        ...
        while (pathNode.getPredecessor() != null) {
            for (Connection edge : pathNode.getIncomingEdges()) {
                if (edge.getSourceNode().getIdentifier().equals(pathNode.getPredecessor().getIdentifier())) {
                    path.add(0, edge);
                }
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Time taken to import and generate graph from tmdb dataset: " + (end - start) / 1000000
        ...
        while("a".equals("a")) {
            ...
            Double deltaSteppingShortestPathDist = ds.calculateShortestPath(g, sourceNode, targetNode, delta);
            end = System.nanoTime();

            System.out.println("The shortest distance between " + sourceNode.getName() + " and " + targetNode.getName() + " is " + deltaSteppingShortestPathDist + ": ");
    ...
    private static AgentNode readActor(TMDBGraph g, Scanner in) {
        ...
        while (node == null) {
            ...
            node = g.getNodes().stream().filter(n -> n.getName().equalsIgnoreCase(name)).findFirst().orElse(null);
            if (node != null) {
                return node;
            } else {
